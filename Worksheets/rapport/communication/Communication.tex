...
In order to evaluate the performance of the new communication protocol, measurements of jitter and packet loss were made for different frequencies. To quantify the improvements made an attempt was made to measure the performances of the previous communication. However as the communication does not have the expected behavior when increasing the refresh rate the results could not be used, details are available in \secref{sec_tcp_mes}. The only comparison between the two protocols that can be made with available data is between the maximum refresh rate that can be reached.
