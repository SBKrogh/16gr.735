...
The problem with the communication latency can be solved in different way, where some of them are listed below.\todo{More analyses is required}
...
\todo{this is not data compressing}
...
\end{verbatim}
\todo{? elaborate string}
This illustrate a read out of four different motor values.
\todo{In the analysis you need to consider the entire system (robot) and describe the type that are used for the data}
...
The low level driver connects to one sbRIO board using a TCP/IP socket and launch a loop in a thread to handle the communication. The data exchange is made using a JSON\todo{JSON? first time mentioned?} stream as described in the previous section. The structure of the code can be seen in figure\figref{original_driver}.
...
\label{new_driver}
\end{figure}
\todo{elaborate the above figure}
\todo{We still need to define the initialization protocol and the packet loss handling}

As explained in \secref{data_compress}, when the JSON Stream was removed we decided that we would only send numerical data. And as we keep goal 3. in mind, we need to receive the position, velocity and effort as float and one boolean for each motor and to send a setpoint as a double and a boolean for each motor.\todo{you may still compress the data}
The received values must be made available as double for the higher levels by storing them in the vector of doubles. For simplicity we decided that the packet should be following the structure described in \figref{received_packet}. To reach the desired vectors,the received bitcode is copied in a float variable that we convert in a double and then the value in the vector is updating using this double.
...
The structure of the sent packet is shown in~\figref{sent_packet}. To build the packet, the same logic as before is used. The bitcode of the numerical values is copied into a buffer and the buffer is sent.
