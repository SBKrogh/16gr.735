...
The low level driver connects to one sbRIO board using a TCP/IP socket and launch a loop in a thread to handle the communication. The data exchange is made using a JSON files as previously explained. The structure of the code can be seen in figure\figref{original_driver}.
...
\node[box] (Check) at ($(-4,-3)+(Initialization)$) {Check if there is\\a new control signal};
\node[box] (Send) at ($(0,-3)+(Check)$) {Send};
\node[box] (Increment) at ($(0,-3)+(Send)$) {Increment the counter};
\node[box] (Connection_Timeout) at ($(0,-3)+(Increment)$) {if counter > 10\\throw an error};
\node[box] (Sleep) at ($(0,-3)+(Connection_Timeout)$) {Sleep};
