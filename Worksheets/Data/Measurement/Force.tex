...
\subsection*{Electric test setup}
For making the force measurement for the EndoWrist a simple hardware construction was build. This setup can be seen on \figref{fig:arduino_loadcell}
...
The load-cell's output is amplified and send to an Arduino which calculate the force applied to the load-cell from the output. The force is then translated to a corresponding voltage output, $4\cdot\frac{mV}{g}$. Since the Arduino does not have a DAC, the voltage is translated to a binary code and send to a "Max531 DAC" with a SPI communication. The voltage output from the DAC is read at the sbRIO board. In parallel, the sbRIO board also samples data such as the current send to the motor, motor position and a time stamp.

\todo{We probaly have to go through the mA applied in each section as 1200 mA and the figures does not seem to fit each other.}
