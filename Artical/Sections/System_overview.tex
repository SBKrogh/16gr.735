...
In order to connect software components responsible for communicating with hardware and the ones responsible for the control algorithm and estimation Robot Operating System (ROS) is used.
...
The end-effector is manipulated by the four wheels seen on Figure \ref{fig:Endo_plates}. The Endowrist is cable driven, which provides the opportunity of making it small but it also makes the system nonlinear due to dry friction.
