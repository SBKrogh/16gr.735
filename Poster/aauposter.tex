...
\begin{posterbox}[name=asd,below=intro]{Communication}
...
The size of the packet was reduced by sending binary representation of the numerical values instead of JSON files. By doing so, the size of the packets was reduced by 85%.
The transport protocol is replaced by UDP as it allows higher bandwidth than TCP
...
\begin{posterbox}[name=results_and_simulations,column=1,span=2,below=Control]{Results and simulations}
\input{results.tex} 
\end{posterbox}
...
\begin{posterbox}[name=references,column=3,above=bottom,,below=conclusion]{References}
