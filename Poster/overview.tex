...
The test system shown on \figref{fig:overview} controls the da Vinci end-effector detached from the system. The control structure presented on ----------- has high computation demand which cannot be satisfied by the embedded system. Therefore the haptic device needs to interface with the embedded system through a computer running the robotic operating sytem (ROS), that also provides the required computation power.
\end{minipage}
\begin{minipage}{\textwidth}
	\begin{figure}[H]
		\resizebox{\textwidth}{!}{
